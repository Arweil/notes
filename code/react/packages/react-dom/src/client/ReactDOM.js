...
ReactWork.prototype.then = function(onCommit: () => mixed): void {
  // 如果_onCommit函数执行过了，那么就直接执行onCommit
  if (this._didCommit) {
    onCommit();
    return;
  }
  ...
  callbacks.push(onCommit); // 缓存在this._callbacks
};
ReactWork.prototype._onCommit = function(): void {
  // 保证callbacks列表中的函数只执行一次
  if (this._didCommit) {
    return;
  }
  this._didCommit = true; // _onCommit函数只能玩一次，_didCommit无法变回false
  const callbacks = this._callbacks;
  if (callbacks === null) {
    return;
  }
...
function legacyRenderSubtreeIntoContainer(
  parentComponent: ?React$Component<any, any>,
  children: ReactNodeList, // React节点
  container: DOMContainer, // 需要挂载的DOM节点
  forceHydrate: boolean, // 是否强制融合
  callback: ?Function,
) {
  if (__DEV__) {
  ...
  let root: Root = (container._reactRootContainer: any); // 获取缓存的ReactRoot对象
  if (!root) {
    // Initial mount
